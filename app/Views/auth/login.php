...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .card .sub{color:var(--muted);font-size:14.5px;margin-top:6px;margin-bottom:26px;}
  ...
  .flash{display:flex;align-items:center;gap:9px;border:1px solid var(--border);border-left:3px solid #8A5A1A;color:#8A5A1A;
    background:var(--surface);border-radius:11px;padding:11px 14px;font-size:13.5px;margin-bottom:18px;}
  .flash.ok{border-left-color:var(--forest);color:var(--forest);}
...
<body>
  ...
  <div class="wrap">
    <div class="card">
      ...
      <div class="sub">Masuk untuk melanjutkan membaca di LIBRIS.</div>

      <?php if (session('error')): ?><div class="flash"><i class="ti ti-alert-triangle" style="font-size:16px"></i> <?= esc(session('error')) ?></div><?php endif; ?>
      <?php if (session('success')): ?><div class="flash ok"><i class="ti ti-circle-check" style="font-size:16px"></i> <?= esc(session('success')) ?></div><?php endif; ?>

      <form action="<?= base_url('login') ?>" method="post">
        <?= csrf_field() ?>
        <div class="field"><label>Email</label><input type="email" name="email" value="<?= esc(old('email')) ?>" placeholder="kamu@email.com" required autofocus></div>
        <div class="field"><label>Kata sandi</label><input type="password" name="password" placeholder="••••••••" required></div>
        <button type="submit" class="btn">Masuk <i class="ti ti-arrow-right" style="font-size:16px"></i></button>
      </form>

      <div class="alt">Belum punya akun? <a href="<?= base_url('register') ?>">Daftar gratis</a></div>
    </div>
  </div>
